...
dev_t dev_num;	//will hold major number that kernel gives us //name--> appears in /proc/devices "mydevicename"
...
static int device_open(struct inode *inode, struct file *filp)
{
	...
	{
		printk(KERN_ALERT "mydevicename : could not lock device during open");
		return -1;
	}
	printk(KERN_INFO "mydevicename : odevice opened ");
	return 0;
}
	//(8) called upon user close
static int device_close(struct inode *inode, struct file *filp)
{
	...
	printk(KERN_INFO "mydevicename: closed device");
	return 0;
}

//(9) tell the kernel which function to call when user operates on our device  file
struct file_operations fops = {
	.owner = THIS_MODULE, //prevent unloading of this module when operations are in use
	.open = &device_open, //points to the method top call when opening the device
...
static int driver_entry(void)
{
	...
	{	//alloc returns negative number if there is an error
		printk(KERN_INFO "mydevicename is failed to allocate major number\n");
		return ret;
	}

		major_number = MAJOR (dev_num); //extracts mjr num stored in dev_num structure
		printk(KERN_INFO "mydevicename : najor number is %d ",major_number);
		...
		if(ret < 0)
		{
			printk(KERN_ALERT "mydevicename: unable to add cdev to kernel \n");
			return ret;
		}
		//step ----------------
...
static void driver_exit(void)	//step(4) exit module, in this unregister everything yove registerd 
{
	...
	printk(KERN_ALERT "mydevicename: unload module ");
}
